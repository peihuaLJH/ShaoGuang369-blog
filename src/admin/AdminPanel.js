        ...
        <>
          ...
          {type === 'blog' && (
            <div className="form-group">
              ...
              {form.coverImage && (
                <img src={form.coverImage.startsWith('http') ? form.coverImage : `http://localhost:5000${form.coverImage}`}
    ...
    <div>
      ...
      <table className="admin-table">
        ...
        <tbody>
          {links.map(l => (
            <tr key={l._id}>
              <td><img src={l.avatar ? (l.avatar.startsWith('http') ? l.avatar : `http://localhost:5000${l.avatar}`) : ''} alt="" style={{ width: 32, height: 32, borderRadius: '50%' }} /></td>
              <td>{l.name}</td>
...
const SettingsManager = ({ showToast, onSettingsChange }) => {
  ...
  const avatarPreview = settings.avatar ? (settings.avatar.startsWith('http') ? settings.avatar : `http://localhost:5000${settings.avatar}`) : '';
